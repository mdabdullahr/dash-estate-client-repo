...
const AgentHome = ({
  ...
  return (
    <div>
      ...
      <section className="mt-10">
        <h3 className="text-xl font-semibold text-[#14203e] mb-4">Recent Offers</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {recentOffers.map((offer) => (
          <div key={offer._id} className="bg-white rounded-lg shadow-sm hover:shadow-xl transition duration-300 overflow-hidden p-4 border border-orange-500">
            <h4 className="font-bold">{offer.propertyTitle}</h4>
            <p className="text-sm text-gray-600">Buyer: {offer.buyerName}</p>
            <p className="text-sm text-gray-600">Email: {offer.buyerEmail}</p>
            <p
